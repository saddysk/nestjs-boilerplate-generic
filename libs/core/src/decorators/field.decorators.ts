...
import { IsBigInt, PositiveBigInt, MinBigInt, MaxBigInt } from 'class-validator-extended';
import {
  ArrayNotEmpty,
  IsArray,
  IsBoolean,
...
export function NumberField(
  ...
  const { each, int, minimum, maximum, isPositive, swagger, ...swaggerOptions } = options;

  if (swagger !== false) {
    decorators.push(
      ApiProperty({ type: Number, minimum, maximum, example: int === false ? 1.2 : 1, ...swaggerOptions })
    );
  }
...
export function StringField(
  options: Omit<ApiPropertyOptions, 'type'> & IStringFieldOptions & { each?: boolean } = {}
): PropertyDecorator {
  const { swagger, each, toLowerCase, toUpperCase, isArray, ...swaggerOptions } = options;
...
export function URLField(
  ...
  return applyDecorators(StringField(options), IsUrl({ require_tld: false, ...(urlOptions ?? {}) }));
}
...
export function URLFieldOptional(
  ...
  return applyDecorators(IsOptional(), URLField({ required: false, ...options }, urlOptions));
}
...
export function ObjectFieldOptional(
  ...
  return applyDecorators(IsOptional(), ObjectField(fieldType, { ...options, required: false }));
}
...
export function FileField(
  ...
  if (options?.swagger !== false) {
    decorators.push(ApiProperty({ type: String, format: 'binary', ...options }));
  }

  return applyDecorators(...decorators);
}

export function FileFieldOptional(options: Omit<ApiPropertyOptions, 'type'> = {}): PropertyDecorator {
...
export function BigIntField(
  ...
  const decorators = [Transform((val) => BigInt(val.value))];

  // eslint-disable-next-line @typescript-eslint/naming-convention
  const { each, minimum, maximum, isPositive, swagger } = options;

  if (swagger !== false) {
    decorators.push(ApiProperty({ type: BigInt, example: '9223372036854775807', ...options }));
  }
  ...
  decorators.push(IsBigInt({ each }));

  if (isNumber(minimum)) {
    decorators.push(MinBigInt(minimum, { each }));
  }

  if (isNumber(maximum)) {
    decorators.push(MaxBigInt(maximum, { each }));
  }

  if (isPositive) {
    decorators.push(PositiveBigInt({ each }));
  }
...
export function BigIntFieldOptional(
  ...
  return applyDecorators(IsOptional(), BigIntField({ required: false, ...options }));
}

// export function isValidAddress(value: unknown) {
